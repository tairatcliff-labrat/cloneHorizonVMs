...
"@		
	...
	try	{
		...
		if($SAN)
		{
			Write-Debug "certreq -adminforcemachine -attrib `"SAN:$SAN`" -submit -config `"$CAName`" certreq.req $CN.cer"
			if($CAName)
			{
				Invoke-Expression -Command "certreq  -adminforcemachine -attrib `"SAN:$SAN`" -submit -config `"$CAName`" certreq.req $CN.cer"
			}
			else
			{
				Invoke-Expression -Command "certreq  -adminforcemachine -attrib `"SAN:$SAN`" -submit certreq.req $CN.cer"	
			}
		}
		else
		{
			Write-Debug "certreq -adminforcemachine -submit -config `"$CAName`" certreq.req $CN.cer"
			if($CAName)
			{
				Invoke-Expression -Command "certreq -adminforcemachine -submit -config `"$CAName`" certreq.req $CN.cer"
			}
			else
			{
				Invoke-Expression -Command "certreq -adminforcemachine -submit certreq.req $CN.cer"		
			}
		}
        ...
        if($FriendlyName){
            ...
            (Get-ChildItem -Path "Cert:\LocalMachine\My\$($cert.Thumbprint)").FriendlyName = "$FriendlyName"
            Write-Host "Set certificate friendly name to $FriendlyName"
        }
